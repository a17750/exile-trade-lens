...
后台以 `chrome.runtime.getManifest().version` 标记当前报告批次。发生以下事件时会清空
`missingRecords`、重置角标，并记录新的 `missingRecordsBuildVersion`：

- 首次安装扩展。
- 安装版本号不同的新代码包。
- 在开发者模式点击扩展“重新加载”；即使版本号暂时相同，`onInstalled:update` 也会强制开启新批次。

普通浏览器重启和 Service Worker 自动休眠/唤醒不会清空同一代码批次的记录。远程翻译数据更新只会
消解已经获得翻译的项目，也不会被误判成代码更新。人工修正和设置不在清理范围内。
