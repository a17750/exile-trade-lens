...
1. 本機同版本 `Content.ggpk`：按穩定內部鍵配對基礎物品、技能、標籤、數值模板、詞綴和命名組件；只讀解析，不修改遊戲檔案。
2. 英服/台服官方 Trade API：以相同 stable ID 或語義 ID 校驗 GGPK，並補充 GGPK 尚未解析或不存在的 Trade 專屬資料。
3. 專案人工覆蓋：`data/manual-overrides.json`，必須寫 `expectedEnglish`、來源和適用領域。
4. 受鎖定版本的第三方名稱表：只作人工查閱和衝突審計，不進入正式回退鏈。
5. 未翻譯回退：證據不足時保留完整英文並進入漏譯報告，不以組件猜測補齊。

完整規範見 [翻譯來源優先級](SOURCE-PRIORITY.md)。Trade stable ID 是運行時定位鍵，不代表 Trade 譯文高於 GGPK。
