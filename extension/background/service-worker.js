...
const DEFAULT_SETTINGS = {
    ...
    mode: "bilingual",
    autoUpdate: true,
    remoteManifestUrl: "https://raw.githubusercontent.com/a17750/exile-trade-lens/main/extension/data/remote-manifest.json",
};
...
async function loadBundledDataset() {
    ...
    if (!response.ok)
        throw new Error(`内置词库读取失败：${response.status}`);
...
function applyLocalOverrides(dataset, overrides = {}) {
    ...
    for (const [key, value] of Object.entries(overrides.static ?? {})) {
        const text = overrideText(value);
        if (text)
            dataset.static.entries[key] = text;
    }
...
function assertManifestUrl(url) {
    ...
    if (!allowed)
        throw new Error("更新地址仅允许 GitHub Raw 或 jsDelivr HTTPS 地址");
}
...
function validateDataset(dataset) {
    ...
    if (statEntries.some(([, entry]) => typeof entry !== "object" ||
        (entry?.text && !entry?.english) ||
        (entry?.renderings != null &&
            (!Array.isArray(entry.renderings) ||
                entry.renderings.some((rendering) => !rendering?.english ||
                    !rendering?.text ||
...
function lookupTranslation(dataset, type, key, renderedUiTexts = null) {
    if (type === "stat")
        return dataset.stats?.entries?.[key]?.text;
    if (type === "item") {
        return dataset.baseItems?.[key] || dataset.fixedNames?.[key] || dataset.items?.[key];
    }
    if (type === "static")
        return dataset.static?.entries?.[key];
    if (type === "filter")
        return dataset.filters?.entries?.[key]?.text;
    if (type === "property")
        return dataset.properties?.[key];
    if (type === "ui") {
        return (dataset.exact?.[key] ||
            dataset.ui?.[key] ||
...
function sanitizeReport(report, { requireTrustedSource = true } = {}) {
    ...
    if (!ALLOWED_MISSING_TYPES.has(type) ||
        !key ||
        !en ||
...
async function recordMissing(reports) {
    ...
    for (const raw of Array.isArray(reports) ? reports : [reports]) {
        ...
        if (!report || lookupTranslation(dataset, report.type, report.key, renderedUiTexts))
            continue;
...
async function reconcileMissing() {
    ...
    for (const [id, record] of Object.entries(missingRecords)) {
        const catalogOnly = ["stat", "item", "static", "filter"].includes(record.type) &&
            !String(record.context ?? "").startsWith("fetch:");
        if (POE2ZHMissingReportPolicy.shouldDiscardStoredReport(record) ||
            catalogOnly ||
...
async function saveOverride(payload) {
    ...
    if (!report)
        return { ok: false, message: "修正项目格式不正确" };
...
async function setMissingIgnored(id, ignored) {
    const { missingRecords = {} } = await chrome.storage.local.get("missingRecords");
    if (missingRecords[id])
        missingRecords[id].ignored = Boolean(ignored);
    await chrome.storage.local.set({ missingRecords });
    await updateBadge(missingRecords);
    return { ok: true };
}
...
async function getHealth() {
    const { missingRecords = {}, localOverrides = {}, updateStatus = null } = await chrome.storage.local.get(["missingRecords", "localOverrides", "updateStatus"]);
    const records = Object.values(missingRecords).sort((a, b) => String(b.lastSeen).localeCompare(String(a.lastSeen)));
    const overrideCount = Object.values(localOverrides).reduce((sum, group) => sum + Object.keys(group ?? {}).length, 0);
...
async function checkForUpdates(force = false) {
    ...
    try {
        ...
        if (!manifestResponse.ok)
            throw new Error(`更新清单请求失败：${manifestResponse.status}`);
        ...
        if (!dataResponse.ok)
            throw new Error(`词库下载失败：${dataResponse.status}`);
...
async function ensureAlarm() {
    ...
    if (!alarm)
        await chrome.alarms.create(ALARM_NAME, { periodInMinutes: 12 * 60 });
}
