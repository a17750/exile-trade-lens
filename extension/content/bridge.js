(() => {
    ...
    function queueMissing(event) {
        ...
        if (!report || !allowed.includes(report.type))
            return;
        if (!missingPolicy.classifyReport(report).allow)
            return;
    ...
    async function flushMissing() {
        missingTimer = null;
        if (contextInvalidated || !pendingMissing.size)
            return;
    ...
    async function publish() {
        ...
        try {
            ...
            if (contextInvalidated || !response)
                return;
            if (!response.ok)
                throw new Error(response.message || "词库加载失败");
        ...
        const add = (source) => {
            for (const [original, translated] of Object.entries(source ?? {})) {
                if (original && typeof translated === "string" && translated) {
                    if (exactConflicts.has(original))
                        continue;
                    ...
                    knownRenderedTranslations.add(`${translated} (${original})`.replace(/\s+/g, " ").trim());
                }
            }
        };
    ...
    function translateTextNode(node) {
        if (!settings.enabled || !exactTranslations.size || !node.nodeValue?.trim())
            return;
    ...
    function translateAttributes(element) {
        if (!settings.enabled || !exactTranslations.size || !element?.getAttribute)
            return;
        ...
        for (const attribute of TRANSLATABLE_ATTRIBUTES) {
            ...
            if (!original)
                continue;
            if (translatedValues.get(attribute) === original)
                continue;
            ...
            if (translated) {
                ...
                reportUiMissing(original, element, () => element.getAttribute(attribute));
        }
    }
    function translateRoot(root) {
        ...
        if (root.closest?.("script, style, textarea, input, [contenteditable='true']"))
            return;
