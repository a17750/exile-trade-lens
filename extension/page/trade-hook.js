(() => {
    ...
    function readSharedConfig() {
        ...
        try {
            ...
            if (!next?.dataset?.datasetVersion)
                return false;
            config = next;
    ...
    function statEnglish(statId) {
        ...
        return (index.englishById.get(statId) ??
    ...
    function componentIndex(includeBaseItems) {
        ...
        if (componentIndexes[cacheKey])
            return componentIndexes[cacheKey];
    ...
    function composeOfficialName(original, includeBaseItems = false) {
        ...
        function solve(offset) {
            ...
            for (const [english, translated] of buckets.get(original[offset]) ?? []) {
                if (!original.startsWith(english, offset))
                    continue;
    ...
    function composeMagicTypeLine(original, baseType) {
        ...
        if (!original || !baseType || !translatedBase)
            return null;
        ...
        if ((prefix && !translatedPrefix) || (suffix && !translatedSuffix))
            return null;
    ...
    function composeNormalTypeLine(original, baseType) {
        ...
        if (quality?.english !== "Superior {0}" ||
            typeof quality.text !== "string" ||
            !quality.text.includes("{0}")) {
            return null;
        }
        if (original !== quality.english.replace("{0}", baseType))
            return null;
    ...
    function translateOption(option, options) {
        ...
        if (translated)
            option.text = format(translated, option.text);
    }
    function translateDataResponse(response, key) {
        const data = config.dataset;
        if (!data || !Array.isArray(response.result))
            return response;
        if (key === "stats") {
            ...
            for (const group of response.result) {
                for (const entry of group.entries ?? []) {
                    ...
                    const translated = direct || (allPartsTranslated ? [name, type].filter(Boolean).join(" ") : null);
                    if (translated)
                        entry.text = format(translated, original);
                    if (name)
                        entry.name = searchableCatalogAlias(name, originalName);
                    if (type)
                        entry.type = searchableCatalogAlias(type, originalType);
                }
            }
        }
        else if (key === "static") {
    ...
    function translateProperty(property) {
        ...
        if (translated)
            property.name = format(translated, property.name);
        else
            reportMissing("property", plain, plain, "item-property");
    }
    function translateMods(item) {
        ...
        for (const [kind, refs] of Object.entries(hashes)) {
            ...
            if (!Array.isArray(mods) || !Array.isArray(refs))
                continue;
            ...
            for (const ref of refs) {
                ...
                if (!translated || !Array.isArray(indexes)) {
                    ...
                    reportMissing("stat", statId, typeof sample === "object" ? sample?.description : sample, `fetch:${kind}`);
                    continue;
                }
            ...
            item[`${kind}Mods`] = mods.map((mod, index) => {
                ...
                const renderingMatches = refsForIndex.flatMap(({ statId }) => matchingStatRenderings(statId, original).map((rendering) => ({ statId, rendering })));
                const matchingRefs = refsForIndex.filter(({ statId }) => sameStatShape(statEnglish(statId), original));
                const templateCandidates = matchingStatTemplates(original);
                let template = renderingMatches.length === 1 ? renderingMatches[0].rendering.text : null;
                if (!template && matchingRefs.length === 1)
                    template = matchingRefs[0].translated;
    ...
    function translateFetchItem(item) {
        ...
        if (translatedBaseType)
            item.baseType = format(translatedBaseType, originalBaseType);
        else if (originalBaseType) {
            reportMissing("item", originalBaseType, originalBaseType, "fetch:baseType");
        }
        if (originalTypeLine) {
            const directTypeLine = config.dataset.baseItems?.[originalTypeLine] ||
                config.dataset.fixedNames?.[originalTypeLine] ||
                config.dataset.items?.[originalTypeLine];
            const composedTypeLine = item.frameType === 0
            ...
            if (directTypeLine) {
                ...
                reportMissing("item", originalTypeLine, originalTypeLine, "fetch:typeLine:normal-display-unresolved");
            }
        ...
        if (item.name) {
            const translatedName = item.frameType === 1 || item.frameType === 2
                ? composeOfficialName(item.name)
                : fixedNameTranslation(item.name);
            if (translatedName)
                item.name = format(translatedName, item.name);
    ...
    ajaxHooker.hook((request) => {
        if (!request.url.includes("/api/trade2/"))
            return;
        ...
        request.response = async (response) => {
            ...
            if (!config.enabled || !config.dataset || !response.responseText)
                return;
            try {
                if (key) {
                    response.responseText = JSON.stringify(translateDataResponse(JSON.parse(response.responseText), key));
                    document.documentElement.dataset.poe2zhLastEndpoint = key;
                }
                else if (request.url.includes("/api/trade2/fetch")) {
                    response.responseText = JSON.stringify(translateFetchResponse(JSON.parse(response.responseText)));
