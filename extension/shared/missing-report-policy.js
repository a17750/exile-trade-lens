(() => {
    ...
    const EDITABLE_SELECTOR = "input:not([type='button']):not([type='checkbox']):not([type='radio']):not([type='submit']), " +
        "textarea, [contenteditable='true'], [role='textbox'], [aria-autocomplete]";
    const RESULT_OR_IGNORED_SELECTOR = "footer, .search-results, .resultset, .results, .listing, " +
        "[class*='itemPopup'], [data-poe2zh-ignore]";
    const DROPDOWN_SELECTOR = "[role='option'], [role='listbox'], [class*='dropdown'], [class*='option']";
    const INPUT_COMPONENT_SELECTOR = "[role='combobox'], [class*='autocomplete'], [class*='typeahead'], " +
        "[class*='suggest']";
    ...
    function uiRegion(element) {
        if (!element?.closest || element.closest(RESULT_OR_IGNORED_SELECTOR))
            return null;
        if (element.closest(DROPDOWN_SELECTOR))
    ...
    function controlledByEditable(element, documentRef) {
        ...
        const id = listbox?.id;
        if (!id || !documentRef?.querySelectorAll)
            return false;
        for (const controller of documentRef.querySelectorAll("[aria-controls], [aria-owns]")) {
            const controls = `${controller.getAttribute?.("aria-controls") ?? ""} ${controller.getAttribute?.("aria-owns") ?? ""}`.trim().split(/\s+/);
    ...
    function isInputDerived({ element, documentRef, activeEditable, lastEditableAt = 0, now = Date.now(), recentInputMs = 2_500, region = uiRegion(element), }) {
    ...
    function classifyDomCandidate({ raw, element, documentRef, activeEditable, lastEditableAt, now, knownRendered = false, alreadyReported = false, }) {
        const text = normalizeText(raw);
        const region = uiRegion(element);
        if (!region)
            return { allow: false, reason: "outside-static-ui" };
        if (isInputDerived({ element, documentRef, activeEditable, lastEditableAt, now, region })) {
    ...
    function classifyReport(report) {
        ...
        if (source === API_SOURCE && report?.type !== "ui")
            return { allow: true };
        if (source === INPUT_SOURCE)
            return { allow: false, reason: "input-derived" };
    ...
    function shouldDiscardStoredReport(report) {
        if (report?.source === INPUT_SOURCE)
            return true;
        return report?.type === "ui" && report?.context === "dropdown-option" && !report?.source;
    }
    function createDomGuard({ documentRef, onAccept, isKnownRendered = () => false, isAlreadyReported = () => false, markReported = () => { }, delayMs = 1_200, now = () => Date.now(), }) {
        ...
        function consider(raw, element, readCurrent = () => raw) {
            ...
            const timer = setTimeout(() => {
                timers.delete(timer);
                if (pending.get(element)?.timer !== timer)
                    return;
                pending.delete(element);
                if (element.isConnected === false || normalizeText(readCurrent()) !== expected)
                    return;
